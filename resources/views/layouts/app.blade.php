...
<div class="wrapper">
    ...
    <nav class="main-header navbar navbar-expand bg-white navbar-light border-bottom">
                    ...
                    @if($smtpSetting->mail_from_email == 'your email')
                        <div>
                            <a class="text-uppercase"
                               href="{{ route('superadmin.smtp-settings.index') }}"><small>@lang('app.add') @lang('menu.smtpSetting')
                                    <i class="fa fa-arrow-right"></i></small></a>
                            <div class="progress">
                                <div class="progress-bar progress-bar-success progress-bar-striped active"
                                     role="progressbar" aria-valuenow="{{$configCompletePercent}}" aria-valuemin="0"
                                     aria-valuemax="100" style="width:{{$configCompletePercent}}%;"><span
                                            class="sr-only">{{$configCompletePercent}}% Complete</span></div>
                            </div>
                        </div>
                    @elseif(is_null($paymentSetting->paypal_client_id) || is_null($paymentSetting->paypal_secret))
                        <div>
                            <a class="text-uppercase"
                               href="{{ route('superadmin.payment-settings.index') }}"><small>@lang('app.add') @lang('menu.paymentSettings')
                                    <i class="fa fa-arrow-right"></i></small></a>
                            <div class="progress">
                                <div class="progress-bar progress-bar-success progress-bar-striped active"
                                     role="progressbar" aria-valuenow="{{$configCompletePercent}}" aria-valuemin="0"
        ...
        <div class="bar__module">
            <select name="language_selector" class=" selectpicker language-switcher" id="language_switcher_admin">
                @forelse ($languageSettings as $language)
                    <option
                            ...
                            data-content=' <span style="  height:16px; width:16px;  " class="flag-icon  @if($language->language_code == 'en') flag-icon-us @else  flag-icon-{{ $language->language_code }} @endif"></span> {{ ucfirst($language->language_code) }}'>{{ ucfirst($language->language_code) }}</option>
                @empty
                @endforelse
            </select>
        </div>
            ...
            @if(!$user->is_superadmin && !is_null($activePackage) && $activePackage->package->is_trial)
                <li class="nav-item">
                    <a href="{{ route('admin.subscribe.index') }}"
                       class="nav-link text-white btn btn-danger btn-sm pt-0"><small>@lang('modules.dashboard.upgrade')
                            <br>
                            @php
...
<!-- Content Wrapper. Contains page content -->
            ...
            <div class="col-md-12" id="sticky-note-header">
                <div class="row">
                    <div class="col-md-10" style="line-height: 30px">
                        @lang('app.menu.stickyNotes') <span
                                class="badge badge-warning">{{ $stickyNotes->count() }}</span>
                        <a href="javascript:;" onclick="showCreateNoteModal()"
                           class="btn btn-outline-success btn-xs ml-3">
                            <i class="fa fa-plus"></i> @lang("modules.sticky.addNote")
                        </a>
                    </div>
                    <div class="col-md-2">
                        <a href="javascript:;" class="btn btn-default btn-circle ml-2" id="open-sticky-bar"><i
                                    class="fa fa-chevron-up"></i></a>
                        <a style="display: none;" class="btn btn-default btn-circle ml-2" href="javascript:;"
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-danger" data-dismiss="modal"><i
                                class="fa fa-times"></i> @lang('app.cancel')</button>
    ...
    {{--Ajax Modal--}}
    <div class="modal fade bs-modal-md in" id="application-md-modal" role="dialog" aria-labelledby="myModalLabel"
         aria-hidden="true">
    ...
    {{--Ajax Modal Ends--}}
    <div class="modal fade bs-modal-lg in" id="application-modal" role="dialog" aria-labelledby="myModalLabel"
         aria-hidden="true">
        <div class="modal-dialog modal-lg" id="modal-data-application">
            <div class="modal-content">
                <div class="modal-header">
...
<script>
    ...
    $('#language_switcher_admin').change(function () {
        var code = $(this).val();
        var url = '{{ route('admin.changeLanguage', ':code') }}';
