...
class FrontCmsHeader extends Model
{
    ...
    protected static function boot()
    {
        ...
        self::retrieved(function (FrontCmsHeader $model){
            $model->translation = FrontCmsHeaderTranslation::query()
                ->where('front_cms_header_id', $model->getAttribute('id'))
                ->whereHas('language', function ($builder){
                    return $builder->where('language_code', app()->getLocale());
                })->first() ?? FrontCmsHeaderTranslation::query()->first();
        });
    }
    ...
    {
        return $this->hasMany(FrontCmsHeaderTranslation::class);
    }
}
